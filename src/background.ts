...
async function dispatch(
	...
	try {
		const tab = await new Promise<chrome.tabs.Tab | undefined>(
			(resolve) => {
...
async function injectContentScript(tabId: number, action: Actions) {
	try {
		...
		chrome.tabs.sendMessage(tabId, { action }, () => {
			if (chrome.runtime.lastError) {
				console.error(
					"Still failed to send message after injection:",
					chrome.runtime.lastError,
				);
			} else {
		...
		try {
			...
			chrome.tabs.sendMessage(tabId, { action }, () => {
				if (chrome.runtime.lastError) {
					console.error(
						"Still failed after fallback injection:",
						chrome.runtime.lastError,
					);
				} else {
					console.log(
						"Message sent successfully after fallback injection",
					);
				}
			});
		} catch (fallbackError) {
...
async function fetchSubmissionResult(
	...
	try {
		...
		if (!response.ok) {
			console.error(
				`Failed to fetch submission result: ${response.status}`,
			);
			return;
		}
		...
		if (
			status === "SUCCESS" &&
			(statusCode === 10 || statusDisplay === "Accepted")
		) {
			...
			console.log(
				"Submission still pending, state:",
				status,
				"display:",
				statusDisplay,
			);
		...
		if (action && !pending.hasDispatched) {
			console.log(
				`Determined final action: ${action} for state: ${status}`,
			);
...
chrome.webRequest.onBeforeRequest.addListener(
	(detail: WebRequestDetails) => {
		...
		if (
			detail.url.includes("leetcode.com") &&
			detail.url.includes("submit") &&
			detail.method === "POST" &&
			!detail.url.includes("/check/")
		) {
...
chrome.webRequest.onCompleted.addListener(
	(detail: WebRequestDetails) => {
		if (
			detail.url.includes("leetcode.com/submissions/detail/") &&
			detail.url.includes("/check/")
		) {
			...
			if (submissionId && pendingSubmissions.has(detail.tabId)) {
				...
				if (
					pending &&
					!pending.hasDispatched &&
					(!pending.submissionId ||
						pending.submissionId === submissionId)
				) {
					console.log(
						`Processing submission check for ID: ${submissionId}`,
					);
					pending.submissionId = submissionId;
...
chrome.webRequest.onBeforeResponse?.addListener(
	(detail: WebRequestDetails) => {
		if (
			...
			if (submissionId && pendingSubmissions.has(detail.tabId)) {
				console.log(
					`Got response for submission check: ${submissionId}`,
				);
